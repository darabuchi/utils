...
func RegisterTopic(name string, messageType interface{}) {
	...
	topicType.Store(name, rt)
}

func RegisterHandel(topicName string, channelName string, handel *Handle) {
	...
	var rt reflect.Type
	{
		rtv, ok := topicType.Load(topicName)
		if !ok {
			log.Panicf("topic %s not register", topicName)
		}
		rt = rtv.(reflect.Type)
	}
	...
	for i := int64(0); i < handel.MaxProcessCnt; i++ {
		go func(clientId int64) {
			logic := func(msg *nsqd.Message) {
				...
				log.Infof("handel %s %s", topicName, channelName)
				...
				log.Infof("[%s(%s)]handel msg %s", topicName, channelName, msg.ID.String())
				...
				if err != nil {
					...
					if err != nil {
						log.Errorf("topic:%s err:%v", topicName, err)
